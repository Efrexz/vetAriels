            ...
            <div className="p-2 lg:px-6">
                <div className="bg-gray-800 rounded-lg shadow-lg p-3 w-full border border-gray-700">
                    <div className="flex flex-col sm:flex-row items-center gap-4 mb-4">
                        <button
                            className="bg-emerald-600 text-white py-2 px-4 rounded-lg hover:bg-emerald-700 flex items-center gap-3 transition-colors font-semibold"
                            onClick={() => navigate(`/pets/create/${id}`)}
                        >
                            <PlusIcon className="w-5 h-5 text-white" />
                            CREAR NUEVA MASCOTA
                        </button>
                        <button className="bg-gray-700 border border-gray-600 text-gray-300 py-2 px-4 rounded-lg hover:bg-gray-600 transition-colors">
                    ...
                    <div className="overflow-x-auto border border-gray-700 rounded-lg hover:cursor-pointer custom-scrollbar">
                        <table className="w-full table-auto bg-gray-800">
                            <thead className="bg-gray-700">
                                <tr>
                                    <th className="py-2 px-4 text-left border border-gray-600">
                                    ...
                                    {tableHeaders.map((header) => (
                                        <th key={header} className="py-2 px-4 border border-gray-600 text-gray-300 text-center">
                                            {header}
                                        </th>
                                    ))}
                                </tr>
                                ...
                                {petsByOwner.length > 0 ? (
                                    petsByOwner.map((pet) => (
                                        <tr key={pet.id} className="bg-gray-800 hover:bg-gray-700 transition-colors duration-200">
                                            ...
                                            <td className="py-2 px-4 border border-gray-600 text-center text-gray-400 text-sm whitespace-nowrap">
                                                <span className="block">{pet.registrationDate}</span>
                                                <span className="block text-gray-500">{pet.registrationTime}</span>
                                            </td>
                                            <td className="py-2 px-4 border border-gray-600 text-center text-gray-400 font-medium">{pet.hc}</td>
                                            <td className="py-2 px-4 border border-gray-600 text-center text-gray-400">{pet.petName}</td>
                                            <td className="py-2 px-4 border border-gray-600 text-center text-gray-400 text-sm whitespace-nowrap">{pet.species} - {pet.breed}</td>
                                            <td className="py-2 px-4 border border-gray-600 text-center text-gray-400 text-sm">{pet.sex}</td>
                                            <td className="py-2 px-4 border border-gray-600 text-center text-gray-400 text-sm whitespace-nowrap">{pet.birthDate}</td>
                                            <td className="py-4 px-4 border border-gray-600">
