...
function Discharges() {
    ...
    return (
        <section className="w-full p-1 md:p-6 bg-gray-950 text-gray-200">
            <h1 className="text-xl md:text-2xl items-center font-medium mb-4 pb-4 border-b-2 border-gray-700 flex">
            ...
            <div className="flex flex-col sm:flex-row items-center space-y-4 sm:space-y-0 sm:space-x-4 mb-6 border-b border-cyan-500 pb-4">
                <button className="w-full sm:w-auto bg-gradient-to-r from-cyan-500 to-emerald-500 text-white py-1.5 px-4 rounded-xl font-bold hover:from-cyan-400 hover:to-emerald-400 transition-all shadow-md">Descargas Emitidas</button>
                <button className="w-full sm:w-auto bg-gray-800 text-cyan-400 py-1.5 px-4 rounded-xl font-bold border border-cyan-500 hover:bg-cyan-500 hover:text-white transition-all shadow-md">Por items</button>
            </div>

            <div className="bg-gray-900 rounded-lg shadow-xl p-3 mb-6 border border-gray-700">
                <div className="px-4 py-2 rounded-lg mb-4 bg-gray-800 border-2 border-cyan-500/30">
                    <div className="flex flex-wrap items-center gap-4 mb-4 ">
                        ...
                        <input
                            type="date"
                            className="w-full md:w-[250px] py-1 px-4 border border-gray-600 rounded-lg bg-gray-700 text-gray-200 focus:outline-none focus:ring-1 focus:ring-cyan-500 transition-colors hover:border-cyan-500"
                            value={filterDate}
                            onChange={(e: ChangeEvent<HTMLInputElement>) => setFilterDate(e.target.value)}
                        />
                        <button
                            className="w-full md:w-auto border border-gray-700 text-white bg-rose-600 py-1 px-4 rounded-xl hover:bg-rose-700 flex items-center justify-center gap-2 transition-colors"
                        ...
                        <thead className="bg-gray-700 border-b border-gray-700">
                            <tr>
                                {tableHeaders.map((header) => (
                                    <th key={header} className="py-1 px-4 text-center border border-gray-600 font-bold text-gray-300 text-sm">
                                        {header}
                                    </th>
                                ))}
                            </tr>
