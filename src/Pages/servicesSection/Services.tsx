...
function Services() {
    ...
    return (
        <section className="w-full p-6 bg-gray-950 text-gray-200">
            <h1 className="text-xl sm:text-2xl font-medium mb-4 pb-4 border-b-2 border-cyan-500 flex items-center">
                <KitMedical className="w-8 h-8 sm:w-9 sm:h-9 mr-3 text-cyan-400 drop-shadow-lg" />
            ...
            <div className="bg-gray-900 rounded-lg shadow-xl p-3 mb-6">
                <div className="p-4 rounded-lg mb-6 bg-gray-800 border-2 border-cyan-500/50">
                    <div className="flex flex-col md:flex-row items-center space-y-4 md:space-y-0 md:space-x-2 mb-4">
                        ...
                        <button
                            className="border border-gray-700 text-white bg-emerald-600 py-1 px-4 rounded-lg hover:bg-emerald-700 flex items-center gap-2 w-full md:w-auto transition-colors"
                        ...
                        {filterOptions.map((filter) => (
                            <div key={filter.type} className="w-full sm:w-[48%] lg:w-[30%]">
                                <select
                                    ...
                                    className="mt-1 w-full rounded-lg hover:border-cyan-500 focus-within:border-cyan-500 border border-gray-600 bg-gray-700 text-gray-200 sm:text-sm py-1 px-2"
                                >
                        ...
                        <thead className="bg-gray-700">
                            <tr>
                                {tableHeaders.map((header) => (
                                    <th key={header} className="py-1 px-4 text-center border-2 border-gray-600 font-bold text-gray-300 text-sm">
                                        {header}
                                    </th>
                                ))}
                            </tr>
                        ...
                        <tbody>
                            {filteredServices.map((service) => (
                                <tr key={service.id} className="hover:bg-gray-700 transition-colors">
                                    <td className="px-4 text-center border border-gray-600 text-gray-400">{service?.id.slice(0, 8).toUpperCase()}</td>
                                    ...
                                    <td className="px-4 text-center border border-gray-600 text-gray-400">{service?.serviceName}</td>
                                    <td className="px-4 text-center border border-gray-600 text-gray-400">{service?.line}</td>
                                    <td className="px-4 text-center border border-gray-600 text-gray-400">{service?.category}</td>
                                    <td className="px-4 text-center border border-gray-600 text-gray-400">{service?.salePrice}</td>
