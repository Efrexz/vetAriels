...
function DashBoard() {
    ...
    return (
        <main className="container mx-auto p-4 sm:p-6 bg-gray-950 text-gray-50 min-h-screen">
            <h1 className="text-3xl sm:text-4xl font-extrabold text-white mb-6 tracking-wide border-b border-cyan-500 pb-3">
            ...
            <section className="grid grid-cols-1 xs:grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4 sm:gap-8 pt-6">
                {dashBoardCategories.map((category) => (
                    <Link key={category.link} to={category.link}>
                        <div className={
                            `relative bg-gray-900 rounded-2xl flex flex-col items-center justify-center p-4 sm:p-6 min-h-[140px] sm:min-h-[160px] cursor-pointer
                            transform transition-all duration-300 hover:scale-105 hover:shadow-xl
                            border-2 border-transparent hover:border-cyan-500/50 group`
                        }>
