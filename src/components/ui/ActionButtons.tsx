...
function ActionButtons({
    ...
    return (
        <div className={`flex flex-col md:flex-row ${mode === "modal" ? "justify-end gap-4  pt-4" : "justify-between items-center bg-gray-900 py-3 px-4 shadow-xl rounded-b-lg border-t-2 border-gray-700 gap-4"}`}>
            <button
                type='button'
                className={`bg-gray-700 border border-gray-600 text-xs md:text-sm lg:text-base text-gray-200 py-2 px-4 rounded-lg hover:bg-gray-600 transition-colors flex items-center gap-2 w-full md:w-auto`}
                onClick={onCancel}
            >
