...
function SideBarMenu({ toggleSideMenu }: SideBarMenuProps) {
    ...
    return (
        <ul className="space-y-2 w-full pt-8 lg:pt-0 pl-4 lg:pl-0 pb-6 md:pb-0 text-gray-200">
            {categories.map((category) => (
                !category.subCategories ? (
                    <li key={category.name} className="group">
                        <Link
                            to={category.path}
                            className="flex items-center gap-2 rounded-full px-4 py-2 text-sm lg:text-[18px] font-medium transition-colors duration-300 hover:bg-cyan-500/20 hover:text-cyan-300"
                            onClick={toggleSideMenu}
                        >
                            <category.icon className="w-4 lg:w-5 h-4 lg:h-5 text-gray-400 group-hover:text-cyan-300 transition-colors duration-300" />
                            {category.name}
                        </Link>
                    </li>
                ) : (
                    <li key={category.name} className="group">
                        <details className="group [&_summary::-webkit-details-marker]:hidden">
                            <summary className="flex cursor-pointer items-center rounded-full px-4 py-2 transition-colors duration-300 hover:bg-cyan-500/20 hover:text-cyan-300">
                                <span className="flex gap-2 items-center text-sm lg:text-[18px] font-medium">
                                    <category.icon className="w-4 lg:w-5 h-4 lg:h-5 text-gray-400 group-hover:text-cyan-300 transition-colors duration-300" />
                                    {category.name}
                                </span>
                            ...
                            <ul className="mt-2 space-y-1 px-4">
                                {category.subCategories?.map((subCategory) => (
                                    <li key={subCategory.name}>
                                        <Link
                                            to={subCategory.path}
                                            className="flex items-center gap-2 rounded-full px-2 py-2 text-sm lg:text-[18px] font-medium  transition-colors duration-300 hover:bg-cyan-500/20 hover:text-cyan-300"
                                            onClick={toggleSideMenu}
                                        >
                                            {subCategory.icon ? <subCategory.icon className="w-4 lg:w-5 h-4 lg:h-5" /> : null}
                                            {subCategory.name}
                                        </Link>
                                    </li>
                                ))}
